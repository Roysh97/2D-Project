...
public class Zombie03 : MonoBehaviour
{
    ...
    int timeToAttack = 0;   // For enemy sound attack
    ...
    void Update()
    {
        if (isAlive)
        {
            ...
            else if (playerDis < range && dist)  // When the player in range
            {
                ...
                if (zombieScale > transform.position.x && playerIsDead.GetComponent<PlayerEngine02>().playerAlive == true)    // Check what is the player scale     in other word if player goes right zombie follows him to the right
                {
                    transform.localScale = new Vector2(1, transform.localScale.y);
                    speed = 0.5f;
                    transform.position = new Vector2(transform.position.x + speed * Time.deltaTime, transform.position.y);
                }
                else if (zombieScale < transform.position.x && playerIsDead.GetComponent<PlayerEngine02>().playerAlive == true)                 // in other word if player goes left zombie follows him to the left
                {
                    transform.localScale = new Vector2(-1, transform.localScale.y);
                    speed = 0.5f;
                    transform.position = new Vector2(transform.position.x - speed * Time.deltaTime, transform.position.y);
            ...
            if (dist == false)        // Condition for attack time   zombie stop following the player and attack
            {
                timeToAttack++;
                if (timeToAttack == 80)
                {
    ...
    {
        if (collision.gameObject.tag == "Player" && isAlive)      // If  zombie touch the player the condition for attack is possible
        {
            dist = false;
            Debug.Log("Attack!");
            soundEffect = true;
