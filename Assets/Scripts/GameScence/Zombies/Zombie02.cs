...
public class Zombie02 : MonoBehaviour
{
    ...
    void Update()
    {
        if (isAlive)
        {
            ...
            else if (playerDis < range && dist)       // When the player in range
            {
                ...
                if (zombieScale > transform.position.x && playerIsDead.GetComponent<PlayerEngine02>().playerAlive == true)    // Check waht is the player scale
                {
            ...
            if (dist == false)
            {
                ...
                if (zombieScale > transform.position.x)             //  in other word if player goes right zombie follows him to the right
                {
                    transform.localScale = new Vector2(1, transform.localScale.y);
                }
                else if (zombieScale < transform.position.x)               // in other word if player goes left zombie follows him to the left
                {
                    transform.localScale = new Vector2(-1, transform.localScale.y);
                }
            }
    ...
    private void OnTriggerEnter2D(Collider2D collision)     // If  zombie touch the player the condition for attack is possible
    {
