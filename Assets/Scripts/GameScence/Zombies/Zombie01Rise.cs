...
public class Zombie01Rise : MonoBehaviour
{
    ...
    public float anmWalkRange;        // The range between the player and the enemy
    ...
    void Update()
    {
        if (isAlive)
        {
            ...
            // anm.GetCurrentAnimatorStateInfo(0).normalizedTime >= 1 detects if any animation is playing and if not , idle animation and sound will work
            if (anm.GetCurrentAnimatorStateInfo(0).normalizedTime >= 1 && playerDis > range && dist)    // Condition for idle position      // if player is far from the zombie
            {
                anm.SetBool("isIdle", true);
                anm.SetBool("isWalk", false);
                anm.SetBool("isAttack", false);
                anm.SetBool("isRise", false);
            ...
            else if (anm.GetCurrentAnimatorStateInfo(0).normalizedTime >= 1 && playerDis < range && dist)   // When the player in range close to zombie
            {
                ...
                if (zombieScale > transform.position.x && playerIsDead.GetComponent<PlayerEngine02>().playerAlive == true)  // Check what is the player scale
                {
                ...
                else if (zombieScale < transform.position.x && playerIsDead.GetComponent<PlayerEngine02>().playerAlive == true)
                {
                    transform.localScale = new Vector2(-1, transform.localScale.y);
                    speed = 1f;
                    transform.position = new Vector2(transform.position.x - speed * Time.deltaTime, transform.position.y);
